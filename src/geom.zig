...
pub const PGABasis = enum(u2) {
    ...
    pub fn squared(b: PGABasis) Sign {
        return switch (b) {
            .e0 => .@"0",
            .e1 => .@"1",
            .e2 => .@"1",
    ...
    const Blade = []const PGABasis;
    pub const pseudo_vector: Blade = &.{ .e0, .e1, .e2, .e3 };
};

pub const pga_primitive = struct {
...
pub const pga = GA(PGABasis, pga_primitive);
pub const cga = GA(PGABasis, pga_primitive);

pub const CGABasis = enum {
    e1,
    e2,
    e3,
    e4,
    ...
    pub fn squared(b: CGABasis) Sign {
        return switch (b) {
            .e1 => .@"1",
            .e2 => .@"1",
            .e3 => .@"1",
    ...
    const Blade = []const CGABasis;

    pub const pseudo_vector: Blade = &.{
        .e1, .e2, .e3, .e4, .eh,
    };
};

fn GA(Basis: type, primitive: type) type {
    return struct {
        const Blade = Basis.Blade;
        ...
        pub fn sameSign(comptime first_order: []const Basis, comptime second_order: []const Basis) bool {
        ...
        pub fn same(first_order: []const Basis, second_order: []const Basis) bool {
        ...
        pub fn xorBlade(comptime values: Blade, comptime blade: Blade) Blade {
            return comptime blk: {
        ...
        pub fn product(lhs: anytype, rhs: anytype, comptime @"type": ProductType) Product(@TypeOf(lhs), @TypeOf(rhs), @"type") {
            const Left = @TypeOf(lhs);
            const Right = @TypeOf(rhs);
